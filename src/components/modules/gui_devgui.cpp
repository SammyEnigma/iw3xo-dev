...
namespace components
{
	...
	void gui_devgui::menu_tab_collision(game::gui_menus_t& menu)
	{
		...
		if (dvars::r_drawCollision->current.integer != 0)
		{
			...
			SPACING(0.0f, 4.0f);
			ImGui::Text("Amount of brushes to render: %d\nAmount of brushsides to render: %d", game::glob::debug_collision_rendered_brush_amount, game::glob::debug_collision_rendered_planes_amount);
#if DEBUG
			SPACING(0.0f, 4.0f);
			...
			if (ImGui::CollapsingHeader("Material Filtering", ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				ImGui::InputTextMultiline("\n", (char*)game::glob::r_drawCollision_materialList_string.c_str(), game::glob::r_drawCollision_materialList_string.size(), ImVec2(ImGui::GetWindowWidth() - 54.0f, ImGui::GetTextLineHeight() * 16), ImGuiInputTextFlags_ReadOnly);
	...
	void dump_visual_settings()
	{
						...
						fs_basepath)
		{
			...
			utils::replace_all(timestamp, ":", "-");
			utils::replace_all(timestamp, " ", "-");

			const std::string file_name = base_path + "visual_settings-" + timestamp + ".cfg";

			cfg.open(file_name.c_str());
			if (cfg.is_open())
			{
				cfg << "// generated by iw3xo - https://github.com/xoxor4d/iw3xo-dev \n";
				cfg << "// film and lighttweak settings dump\n";
				cfg << "\n";

				cfg << "set r_filmTweakEnable " << r_filmTweakEnable->current.enabled << std::endl;
				cfg << "set r_filmUseTweaks " << r_filmUseTweaks->current.enabled << std::endl;
				cfg << "set r_filmTweakInvert " << r_filmTweakInvert->current.enabled << std::endl;
				cfg << "set r_filmTweakLightTint " << r_filmTweakLightTint->current.vector[0] << " " << r_filmTweakLightTint->current.vector[1] << " " << r_filmTweakLightTint->current.vector[2] << std::endl;
				cfg << "set r_filmTweakDarkTint " << r_filmTweakDarkTint->current.vector[0] << " " << r_filmTweakDarkTint->current.vector[1] << " " << r_filmTweakDarkTint->current.vector[2] << std::endl;
				cfg << "set r_filmTweakDesaturation " << r_filmTweakDesaturation->current.value << std::endl;
				cfg << "set r_filmTweakContrast " << r_filmTweakContrast->current.value << std::endl;
				...
				cfg << "set r_lightTweakSunLight " << r_lightTweakSunLight->current.value << std::endl;

				game::Com_PrintMessage(0, utils::va("Dumped visual settings to: %s", base_path.c_str()), 0);
				cfg.close();
			}
		}
	}
