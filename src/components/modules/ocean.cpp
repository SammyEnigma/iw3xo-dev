...
namespace components
{
	...
	void dump_settings()
	{
						...
						fs_basepath)
		{
			std::string base_path = fs_basepath->current.string;
						base_path += "\\iw3xo\\shader_settings\\";
			...
			// steam to .map file
			ocean.open((base_path + "ocean_settings.txt").c_str());
			...
			game::Com_PrintMessage(0, utils::va("Dumped shader settings to: %s", (base_path + "ocean_settings.txt").c_str()), 0);
			ocean.close();
		}
	}
	...
	void ocean::set_vertexshader_constants(game::GfxCmdBufState* state, game::MaterialShaderArgument* const arg_def)
	{
		if (state->pass->vertexShader && !utils::q_stricmp(state->pass->vertexShader->name, "worldfx_ocean"))
		{
			...
			{
				const float constant[4] = { _WindDirection[0], _WindDirection[1], _HeightIntensity, _WaveAmplitudeFactor };
			...
			{
				const float constant[4] = { _WaveSteepness, _TextureTiling, _WaveTiling, _Time };
				(*game::dx9_device_ptr)->SetVertexShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_FILTER_TAP_5)
			{
				const float constant[4] = { _VisibleWaveDist, _HeightMapScale, _HeightMapScroll, 0.0f };
				(*game::dx9_device_ptr)->SetVertexShaderConstantF(arg_def->dest, constant, 1);
			}
		}
	}
	...
	void ocean::set_pixelshader_constants(game::GfxCmdBufState* state, game::MaterialShaderArgument* const arg_def)
	{
		if (state->pass->pixelShader && !utils::q_stricmp(state->pass->pixelShader->name, "worldfx_ocean"))
		{
			...
			{
				const float constant[4] = { _AmbientColor[0], _AmbientColor[1], _AmbientColor[2], _AmbientDensity };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_FILTER_TAP_4)
			{
				const float constant[4] = { _ShoreColor[0], _ShoreColor[1], _ShoreColor[2], _DiffuseDensity };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_FILTER_TAP_5)
			{
				const float constant[4] = { _SurfaceColor[0], _SurfaceColor[1], _SurfaceColor[2], _NormalIntensity };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_FILTER_TAP_6)
			{
				const float constant[4] = { _DepthColor[0], _DepthColor[1], _DepthColor[2], _ShoreFade };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_FILTER_TAP_7)
			{
				const float constant[4] = { _RefractionValues[0], _RefractionValues[1], _RefractionValues[2], _RefractionScale };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_COLOR_MATRIX_R)
			{
				const float constant[4] = { _HorizontalExtinction[0], _HorizontalExtinction[1], _HorizontalExtinction[2], _Distortion };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_COLOR_MATRIX_G)
			{
				const float constant[4] = { _WaterClarity, _WaterTransparency, _RadianceFactor, 0.0f };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_COLOR_MATRIX_B)
			{
				const float constant[4] = { _SpecularValues[0], _SpecularValues[1], _SpecularValues[2], _Shininess };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_DOF_EQUATION_VIEWMODEL_AND_FAR_BLUR)
			{
				const float constant[4] = { _FoamTiling[0], _FoamTiling[1], _FoamTiling[2], _FoamSpeed };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}

			else if (arg_def->u.codeConst.index == game::ShaderCodeConstants::CONST_SRC_CODE_DOF_EQUATION_SCENE)
			{
				const float constant[4] = { _FoamRanges[0], _FoamRanges[1], _FoamRanges[2], _FoamIntensity };
				(*game::dx9_device_ptr)->SetPixelShaderConstantF(arg_def->dest, constant, 1);
			}
